...
class AuthorizerModule {
    ...
    authorize(event, context) {
        ...
        if (principalId) {
            this.buildPolicy(principalId, context, this.retrieveAccountId(event));
            return { success: true };
        }
     ...
     * @param principalId
     */
    buildPolicy(principalId, context, accountId) {
     //sample arn: Sample arn: arn:aws:execute-api:us-west-2:xxxxxxxxx:kvmxspwm7g/*/GET/
        var policy = new PolicyBuilder(principalId, context, accountId).build();
        return policy;
    }
}
